...
export function tryParseAmount(value?: string, currency?: Currency): CurrencyAmount | undefined {
    ...
    try {
        ...
        if (typedValueParsed.toString() !== '0') {
            return currency instanceof Token ? new TokenAmount(currency, JSBI.BigInt(typedValueParsed.toString())) : CurrencyAmount.ether(JSBI.BigInt(typedValueParsed.toString()))
        }
    } catch (error) {
